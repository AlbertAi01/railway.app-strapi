...
export default function WulingMapPage() {
  ...
  const zoomTo = useCallback((level: number) => {
    ...
    setOffset({ x: vw / 2 - cx * level, y: vh / 2 - cy * level });
    setZoom(level);
  }, [zoom, offset]);
  ...
  const panToPoi = useCallback((poi: POI) => {
    ...
    const vw = containerRef.current.clientWidth;
    const vh = containerRef.current.clientHeight;
    const targetZoom = Math.max(zoom, 0.12);
    setZoom(targetZoom);
    setOffset({ x: vw / 2 - poi.px * targetZoom, y: vh / 2 - poi.py * targetZoom });
    setSelectedPoi(poi);
  }, [zoom]);
    ...
    <div className="h-screen flex flex-col bg-black overflow-hidden">
      <div className="h-10 flex items-center gap-3 px-3 bg-[var(--color-surface)] border-b border-[var(--color-border)] shrink-0 z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-[var(--color-accent)] uppercase tracking-wider">Wuling</span>
        </div>
      ...
      <div className="flex-1 flex relative overflow-hidden">
        ...
        >
          ...
          >
            <g id="tiles-layer">
              {tiles.map((t, i) => (
                <image key={i} href={t.href} x={t.x} y={t.y}
          ...
          >
            {clusters.map(cluster => {
              ...
              return (
                <div key={cluster.key} className="absolute pointer-events-auto"
