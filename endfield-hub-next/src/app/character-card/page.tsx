  ...
  return (
    <div className="text-gray-400">
      <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
        <Sparkles className="w-8 h-8 text-[#FFE500]" />
        Character Card Creator
      </h1>
      <p className="text-gray-500 text-sm mb-6">Create and share beautiful character showcase cards</p>
        ...
        <div className="space-y-4">
          <div className="bg-[#111] border border-[#222] rounded-xl p-5">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-bold mb-1.5 text-gray-400">Character</label>
                <select
                  value={selectedCharacter}
                  onChange={(e) => setSelectedCharacter(e.target.value)}
                  className="w-full px-3 py-2.5 bg-[#0a0a0a] border border-[#222] rounded-lg focus:outline-none focus:border-[#FFE500] text-white text-sm"
                >
              ...
              <div>
                <label className="block text-xs font-bold mb-1.5 text-gray-400">Custom Text</label>
                <input
                  type="text"
                  value={customText}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold mb-1.5 text-gray-400">Background</label>
                  <div className="flex gap-2">
                    ...
                    <input
                      ...
                      value={backgroundColor}
                      onChange={(e) => setBackgroundColor(e.target.value)}
                      className="flex-1 px-2 py-2 bg-[#0a0a0a] border border-[#222] rounded text-xs focus:outline-none focus:border-[#FFE500] text-white font-mono"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold mb-1.5 text-gray-400">Accent</label>
                  <div className="flex gap-2">
                    <input
                      type="color"
                      value={accentColor}
              ...
              {character && (
                <div>
                  <label className="block text-xs font-bold mb-1.5 text-gray-400">Quick Themes</label>
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setBackgroundColor('#1a1a2e'); setAccentColor(elementColor); }}
                      className="flex-1 py-2 text-xs rounded-lg border border-[#222] hover:border-[#FFE500] transition-colors"
                    ...
                    <button
                      ...
                      className="flex-1 py-2 text-xs rounded-lg border border-[#222] hover:border-[#FFE500] transition-colors text-[#00BFFF]"
                    >
                      Frost
                    </button>
                    <button
                      onClick={() => { setBackgroundColor('#1a0a0a'); setAccentColor('#FF6B35'); }}
                      className="flex-1 py-2 text-xs rounded-lg border border-[#222] hover:border-[#FFE500] transition-colors text-[#FF6B35]"
                    >
                      Ember
                    </button>
                  </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#FFE500] text-black font-bold rounded-lg hover:bg-[#FFE500]/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
                >
                ...
                <div className="relative">
                  <button
                    ...
                    disabled={!character || isExporting}
                    className="py-2.5 px-4 bg-[#222] text-white font-bold rounded-lg hover:bg-[#333] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
                  >
                  ...
                  {showShareMenu && character && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-[#111] border border-[#333] rounded-lg shadow-xl z-50 overflow-hidden">
                      <button onClick={shareToTwitter} className="w-full px-4 py-3 text-left text-sm hover:bg-[#222] flex items-center gap-3 text-gray-300">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        Post on X
                      </button>
                      <button onClick={shareToReddit} className="w-full px-4 py-3 text-left text-sm hover:bg-[#222] flex items-center gap-3 text-gray-300">
                      ...
                      <button onClick={shareToDiscord} className="w-full px-4 py-3 text-left text-sm hover:bg-[#222] flex items-center gap-3 text-gray-300">
                      ...
                      <button onClick={copyToClipboard} className="w-full px-4 py-3 text-left text-sm hover:bg-[#222] flex items-center gap-3 text-gray-300 border-t border-[#222]">
                      ...
                      {typeof navigator !== 'undefined' && 'share' in navigator && (
                        <button onClick={nativeShare} className="w-full px-4 py-3 text-left text-sm hover:bg-[#222] flex items-center gap-3 text-gray-300 border-t border-[#222]">
                          <Share2 className="w-4 h-4" />
                          More Options...
                        </button>
                      )}
          ...
          {character && (
            <div className="bg-[#111] border border-[#222] rounded-xl p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Max ATK</span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Max DEF</span>
