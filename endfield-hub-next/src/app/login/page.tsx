...
export default function LoginPage() {
  ...
  const handleMagicLink = async () => {
    ...
    try {
      // In a real implementation, this would call your magic link API
      alert(`Magic link sent to ${email}! Check your email.`);
      setIsLoading(false);
    } catch (err) {
      ...
      <div className="max-w-md w-full">
        <div className="mb-8">
          <RIOSHeader
            ...
            icon={<LogIn size={28} />}
          />
          <p className="text-center mt-4">Sign in to access your Endfield Hub account</p>
        </div>

        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] clip-corner-tl p-8">
          {error && (
            <div className="mb-6 p-4 bg-red-900/20 border border-red-500/50 clip-corner-tl text-red-300 text-sm">
              {error}
            </div>
          )}
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              disabled={isLoading}
              className="w-full py-3 px-4 bg-white text-black font-bold clip-corner-tl hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full py-3 px-4 bg-[#5865F2] text-white font-bold clip-corner-tl hover:bg-[#4752C4] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
            >
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 bg-[var(--color-accent)] text-black font-bold clip-corner-tl hover:bg-[var(--color-accent)]/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
          ...
          <div className="mt-6 pt-6 border-t border-[var(--color-border)] text-center text-sm">
            <p>
              ...
              <Link href="/signup" className="text-[var(--color-accent)] hover:underline font-bold">
                Sign up
              </Link>
            </p>
          </div>
